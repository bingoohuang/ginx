...
func TestPopulate(t *testing.T) {
	...
	err := cast.ToStruct(m, "prop", nil)
	it.Error(err)

	err = cast.ToStruct(&x, "prop", nil)
	it.Error(err)
	...
	mapGetter := func(m map[string]string) func(filedName, tagValue string) (interface{}, bool) {
		return func(filedName, tagValue string) (interface{}, bool) {
			return cast.TryFind(filedName, tagValue, func(name string) (interface{}, bool) {
				v, ok := m[name]
				return v, ok
			})
		}
	...
	err = cast.ToStruct(&m, "prop", mapGetter(prop))
	it.Nil(err)
	...
	it.Error(cast.ToStruct(&myx, "prop", mapGetter(prop)))
	it.Error(cast.ToStruct(&myy, "prop", mapGetter(prop)))
}
...
func TestCastAny(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			got, err := cast.To(tt.args.s, tt.args.t)
			if (err != nil) != tt.wantErr {
				t.Errorf("To() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if err != nil {
				if got != cast.InvalidValue {
